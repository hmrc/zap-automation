...
trait ZapTest extends WordSpec {
  ...
  var zapConfig: Config = ConfigFactory.empty()

  def initialiseConfiguration(config: Config) : Unit = {
    zapConfig =  new LoadConfig(config).extractedConfig()
  }

  def alertsToIgnore():List[ZapAlertFilter] = {
    val listofalerts = zapConfig.getConfigList("alertsToIgnore")
    println ("Printing filter cweid at index 0: " + listofalerts.get(0).getString("cweid"))
    List.empty
  }

  def callZapApi(queryPath: String, params: (String, String)*): String = {
    val (status, response) = theClient.get(zapConfig.getString("zapBaseUrl"), queryPath, params: _*)
  ...
  def setUpPolicy(policyName: String): Unit = {
    ...
    if (!zapConfig.getBoolean("testingAnApi")) {
  ...
  def setUpContext(contextName: String): Unit = {
    callZapApi("/json/context/action/includeInContext", "contextName" -> contextName, "regex" -> zapConfig.getString("contextBaseUrl"))
    callZapApi("/json/context/action/excludeAllContextTechnologies", "contextName" -> contextName)

    if (zapConfig.getString("desiredTechnologyNames").nonEmpty) {
      callZapApi("/json/context/action/includeContextTechnologies", "contextName" -> contextName, "technologyNames" -> zapConfig.getString("desiredTechnologyNames"))
    }

    if (zapConfig.getString("routeToBeIgnoredFromContext").nonEmpty) {
      callZapApi("/json/context/action/excludeFromContext", "contextName" -> contextName, "regex" -> zapConfig.getString("routeToBeIgnoredFromContext"))
    }
  }
  ...
  def runAndCheckStatusOfSpider(contextName: String): Unit = {
    callZapApi("/json/spider/action/scan", "contextName" -> contextName, "url" -> zapConfig.getString("testUrl"))
  ...
  def runAndCheckStatusOfActiveScan(contextId: String, policyName: String): Unit = {
    ...
    if (isActiveScanRequired) {
      ...
      callZapApi("/json/ascan/action/scan", "contextId" -> contextId, "scanPolicyName" -> policyName, "url" -> zapConfig.getString("testUrl"))
  ...
  def filterAlerts(allAlerts: List[ZapAlert]): List[ZapAlert] = {
    ...
    if(zapConfig.getBoolean("ignoreOptimizelyAlerts"))
  ...
  def parsedAlerts: List[ZapAlert] = {
    val response: String = callZapApi("/json/core/view/alerts", "baseurl" -> zapConfig.getString("alertsBaseUrl"))
    val jsonResponse = Json.parse(response)
  ...
  "Tearing down the policy, context and alerts" should {
    "complete successfully" in {
      if(zapConfig.getBoolean("tearDown")){
